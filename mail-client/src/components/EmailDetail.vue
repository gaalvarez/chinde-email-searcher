  ...
  <div>
    <div class="flex bg-gray-300">
      <div class="w-1/2 p-4">
        ...
        <div class="text-sm text-gray-700 font-medium">
          To:
          <span class="text-sm text-black font-medium">{{ toFormated }}</span>
        </div>
      </div>
      <div class="w-1/2 text-right p-4">
        <button class="" @click="() => onCloseButtonClick()">
          <svg
        ...
        <div class="text-xs text-gray-500 font-xs">
          {{ props.emailInfo.date }}
        </div>
      </div>
    </div>
    <div class="bg-white p-4">
      <div class="mt-2 text-xl text-black font-medium mb-4">
        Subject: {{ props.emailInfo.subject }}
      </div>

      <div class="text-sm text-gray-700 font-medium">
        <span v-html="bodyHighlight"></span>
      </div>
    </div>

    <!-- toolbar copy function pass to other component -->
    <!-- <div class="flex items-center justify-between w-full">
      <div class="pl-4">
      </div>
      <div class="pr-4">
        <button
          class="transform hover:scale-125 transition duration-150 text-white p-1"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
...
const emit = defineEmits<{
  (e: "emaildetail:close"): void;
}>();

const onCloseButtonClick = () => {
  emit("emaildetail:close");
};

const toFormated = computed(() => props.emailInfo.to.slice(0, 2).join(", "));

const bodyHighlight = computed(() =>
  props.emailInfo.body.replace(
    new RegExp(props.searchTerm, "gi"),
    `<kwr>${props.searchTerm}</kwr>`
  )
);
</script>
